...
%%%%%%%%%%%%%% How many times running ENN
time=300;
MPEE=[];
INTEN_2=[];
fifi=nan*ones(300,95*12); % blank matrix for output
TYYYY='AREA'; %predict area
...
jinanin(1:tititi,4)=T_OD_WS(1:tititi)';% wind speed
jinanin(1:tititi,5)=T_OD_PRE(1:tititi)';%precipitation
jinanin(1:tititi,6)=T_OD_SST(1:tititi)';%sea surface temperature
jinanin(1:tititi,7)=T_OD_SSH(1:tititi)';%sea surface height
jinanin(1:tititi,8)=T_OD_SR(1:tititi)';%solar radiation
jinanin(1:tititi,9)=T_OD_CS(1:tititi)';%current speed(not used)
...
if strcmp(TYYYY,'AREA') %predict'AREA' or 'INTENSITY'
...
kk_1=simout';
fifi(timmm,:)=kk_1;%fifi - output

Cx_11_m=[1:95];%amplitude
Cx_22_m=[1:95];%max
Cx_33_m=[1:95];%min
